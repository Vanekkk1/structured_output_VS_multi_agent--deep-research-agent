...
export const saveMarkdownReport = async (
  title: string,
  content: string,
  query: string,
  sources: string[]
  ...
  try {
    ...
    const sanitizedQuery = query
      ...
      .replace(/\s+/g, "-")
      .substring(0, 50);
    const filename = `research-structured-${timestamp}-${sanitizedQuery}.md`;
...
**Agent:** Structured Output System
...
export const checkAPIKeys = () => {
  ...
  if (!process.env.TAVILY_API_KEY) {
    throw new Error(
      "TAVILY_API_KEY is not set, please set it in the .env file"
    );
  }
};
